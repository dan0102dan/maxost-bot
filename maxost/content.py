...
SUPPORTED_TG = (
    'photo', 'animation', 'sticker', 'video', 'voice',
    'audio', 'video_note', 'document',
)


def split_text(text, limit=3500):
    return [part['text'] for part in chunks(text, limit=limit)]
...
def canonical(payload):
    """Normalize the current queue payload format."""
    return {
        **payload,
...
def from_telegram(message, limit):
    ...
    entities = valid_entities(
        text, message.get('entities', message.get('caption_entities', []))
    )
    attachments = []
    for kind in SUPPORTED_TG:
        ...
        defaults = {
            ...
            'voice': 'voice.ogg',
            'video': 'video.mp4',
            'video_note': 'circle.mp4',
            'audio': 'audio.mp3',
            'animation': 'animation.mp4',
            'sticker': (
                'sticker.tgs' if source.get('is_animated')
                else 'sticker.webm' if source.get('is_video')
                else 'sticker.webp'
            ),
        }
        attachments.append({
            ...
            'name': safe_name(
                source.get('file_name'), defaults.get(kind, 'document.bin')
            ),
            'duration': int(source.get('duration', 0)) * 1000,
            'is_animated': source.get('is_animated', False),
            'is_video': source.get('is_video', False),
            'emoji': source.get('emoji', ''),
            'tg_id': message.get('message_id'),
        })
        break
    ...
    if 'poll' in message:
        ...
        if poll.get('type') == 'quiz':
            raise Rejected(
                'Telegram не передаёт правильный ответ чужой викторины. '
                'Создайте обычный опрос.'
            )
        if any(option.get('media') for option in poll.get('options', [])) or poll.get('media'):
            raise Rejected(
                'Опрос с медиа-вариантами нельзя отправить в MAX. '
                'Создайте текстовый опрос.'
            )
        attachments.append({
            'kind': 'poll',
            'poll': {
                'title': poll['question'],
                'answers': [{'text': option['text']} for option in poll['options']],
                'settings': (
                    (1 if poll.get('is_anonymous', True) else 0)
                    | (2 if poll.get('allows_multiple_answers') else 0)
                    | 4
                    | (8 if poll.get('is_closed') else 0)
                ),
            },
            'tg_id': message.get('message_id'),
        })
    ...
    if not text and not attachments:
        raise Rejected('Этот тип сообщения не поддерживается.')
...
def combine_album(members):
    ...
    for member in sorted(members, key=lambda payload: int(payload['tg_ids'][0])):
        if member.get('text'):
            prefix = '\n\n' if text else ''
            shift = utf16(text + prefix)
            text += prefix + member['text']
            entities.extend(
                {**entity, 'offset': entity['offset'] + shift}
                for entity in member.get('entities', [])
            )
    ...
    if len(attachments) > 10:
        raise Rejected(
            'Альбом содержит больше 10 вложений. Разделите его на несколько.'
        )
    return {
        'text': text,
        'entities': entities,
